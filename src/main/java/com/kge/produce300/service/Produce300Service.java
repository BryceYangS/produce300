...
public class Produce300Service {
	...
	public List<CandidateDTO> retrieveCandidates(String sggCode) throws Exception {
		...
		List<Node> voteRateColumns = IntStream.range(0, row.size())
			.filter(i -> i % 2 == 1)
			.mapToObj(row::get)
			.collect(toList());
		...
		for (int i = 0; i < candidates.size(); i++) {
			...
			CandidateDTO candidateDto = modelMapper.map(candidates.get(i), CandidateDTO.class);
			candidateDto.setOpenVoteRate(openVoteRate);
			candidateDto.setVoteRate(voteRate);
			...
			candidatesWithVoteRate.add(candidateDto);
		}
	...
	public List<CandidateDTO> getAllCandidateData() throws Exception {
		...
		List map = modelMapper.map(candidates, List.class);
